...
public class CPACEPConnector {
  ...
  public CPACEPConnector(ILaunchConfiguration config) {
    ...
    try {
      ...
      e.printStackTrace();
      // logger.log(new Status(IStatus.ERROR, PLUGIN_ID, "Error retrieving attributes from the
      // launch configuration", e)); //$NON-NLS-1$
    }
  }
  ...
  private void initializeBaseCommandLine() {
    ...
    baseCli.add(" -spec " + CPACheckerHome + File.separator + "config" + File.separator
        + "specification" + File.separator + lcSpecification + SPEC_FILE_TYPE);
    baseCli.add(" -config " + CPACheckerHome + File.separator + "config" + File.separator
        + lcConfiguration + CONFIG_FILE_TYPE);
